...
Scanned PDFs requiring OCR are not supported. Pinecone and the embedding provider must be configured before processing can complete. Part 2 can build fact extraction and reconciliation on the traceable `Document -> Page -> Chunk -> Embedding` foundation; Part 1 does not implement those features.